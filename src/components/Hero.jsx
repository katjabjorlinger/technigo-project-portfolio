import katjaImg from "/assets/katja.jpg";


export const Hero = () => {
    return (<section className="hero">
        <div className="hero-container">
          <div className="hero-left">
            <div className="hero-header animate-fadeInUp delay-1">
              <h1>Hi, I’m</h1>
              <h2>Katja Björlinger</h2>
              <h3>Software Developer</h3>
            </div>
            <p className="hero-text animate-fadeInUp delay-3">
              I’m a curious and creative problem-solver with a passion for building thoughtful digital experiences. I enjoy turning complex ideas into something clear, usable, and human. My background combines communication and tech – and I love working in the space where those meet.
            </p>
          </div>
      
          <img src={katjaImg} alt="Katja Björlinger" className="hero-image animate-fadeInUp delay-2" />
        </div>
      </section>



       
    )
    ;
}


